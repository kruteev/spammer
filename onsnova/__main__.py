...
os.chdir(os.path.join(pkg_resources.get_distribution("spammer").location, "spammer"))
...
def update(force: bool = False):
    output = subprocess.run(
        ["pip3", "install", "spammer", "--upgrade"],
        stdout=subprocess.PIPE,
        stderr=subprocess.DEVNULL,
    )
    if force or "Successfully installed spammer" in output.stdout.decode():
        subprocess.run(
            ["pip3", "install", "spammer", "--force-reinstall" if force else "--upgrade"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        os.execlp("spammer", " ".join(sys.argv[1:]) or "--port 8080")
